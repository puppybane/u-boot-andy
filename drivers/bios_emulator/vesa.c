...
static int list_vesa_modes(int mode)
{
	...
	if (regs.x.ax != 0x004f) {
	    printf("VESA call failed\n");
	    return 0;
	}
	
	// Check for VGA signature
	if (*(u32 *)mem != 0x56455341) {
	    printf("VESA mode list not found (%x)\n", *(u32 *)mem);
	    return 0;
	}
	...
	while((*table != 0xffff)) {
		...
		if (found_mode == (mode & 0x3ff)) {
		    printf("Found required VESA mode\n");
		    return 1;
		}
		table++;
	}
	
	printf("Required VESA mode (%d) not found\n", mode);
	return 0;
}
...
static int get_current_vesa_mode()
{
	...
	printf("About to get mode\n");
	BE_int86(0x010, &regs, &regs);
	printf("Got mode %d (0x%x)\n", regs.x.bx, regs.x.bx);
	return regs.x.bx;
}
